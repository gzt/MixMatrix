...
test_that("trying wrong type of input", {
  ...
  expect_error(rmatrixnorm(
    n = "A", mean = matrix(c(100, 0, -100, 0, 25, -1000), nrow = 2),
  ...
  expect_error(rmatrixnorm(
    n = 10, mean = matrix(c(100, 0, -100, 0, 25, -1000), nrow = "A"),
    L = matrix(c(2, 1, 0, .1), nrow = 2),
    R = matrix(c(5, 1, 2, 0, 6, 1, -1, 2, 10), nrow = 3),
    list = FALSE
  ),
  ...
  expect_error(dmatrixnorm(matrix(c(100, 0, -100, 0, 25, -1000), nrow = 2),
    mean = matrix(c(100, 0, -100, 0, 25, -1000), nrow = 2),
    L = matrix(c(2, 1, 0, .1), nrow = 2),
    V = matrix(c("A", 1, 2, 0, 6, 1, -1, 2, 10), nrow = 3),
    log = TRUE
  ),
  "non-numeric",
  ignore.case = TRUE
  ...
  expect_error(MLmatrixt(rmatrixnorm(n = 100, mean = diag(5)), tol = "Q"),
    "non-numeric",
    ignore.case = TRUE
  )
  expect_error(MLmatrixt(rmatrixnorm(n = 100, mean = diag(5)), max.iter = "Q"),
    "non-numeric",
    ignore.case = TRUE
  )
  expect_error(MLmatrixt(rmatrixnorm(n = 100, mean = diag(5)), df = "Q"),
    "non-numeric",
    ignore.case = TRUE
  )
