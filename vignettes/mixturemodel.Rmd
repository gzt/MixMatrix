...
is that one is now including estimates of $ \pi_j $ for $j$ in  $1,2, \ldots, g$, 
the estimated probabilities of group membership for the $g$ groups in each step
and weights $\tau_{ig}$, weights for each observation $i$ and group $g$, where

$\pi_j = \sum_{i = 1}^n \tau_{ij}$ and 
\[ \tau_{ij} = \frac{\pi_j f(x_i, \Theta_j)}{\sum_l^g \pi_l f(x_i, \Theta_l)}\]

The updates on the parameters $\Theta$ are weighted by $\tau_{ij}$. 

The `matrixmixture()` function fits unrestricted covariance matrices currently,
but future features will implement a `t-eigen` type of covariance restriction
capability. 
