...
it is a simple regularized linear/quadratic discriminant function based on the 
likelihood function. As such, the reported posteriors are based on the normal 
distribution.

The `matrixlda` function presumes equal covariance parameters while `matrixqda` 
fits separate covariance parameters.

It is possible to set variance or mean restrictions using the `...` argument. 
These restrictions are applied to all groups.
